...
namespace SistemaCondominioDCES.Controllers
{
    public class ReservaController : Controller
    {
        ...
        public ActionResult Create(Reserva reserva)
        {
            ...
            if (!ok)
            {
                ViewBag.Error = "Ya existe una reserva en ese horario.";
                return View(reserva);
            }
        ...
        public ActionResult Edit(int id)
        {
            ...
            using (SqlConnection conexion = cn.ObtenerConexion())
            {
                string query = "SELECT IdReserva, IdUsuario, IdAreaComun, FechaReserva, HoraInicio, HoraFin, Estado FROM Reservas WHERE IdReserva = @Id";
            ...
            using (SqlConnection conexion = cn.ObtenerConexion())
            {
                SqlCommand cmd = new SqlCommand("SELECT IdUsuario, Nombre FROM Usuarios", conexion);
                conexion.Open();
                SqlDataReader dr = cmd.ExecuteReader();
                while (dr.Read())
                {
            ...
            using (SqlConnection conexion = cn.ObtenerConexion())
            {
                SqlCommand cmd = new SqlCommand("SELECT IdAreaComun, NombreArea FROM AreasComunes", conexion);
                conexion.Open();
                SqlDataReader dr = cmd.ExecuteReader();
                while (dr.Read())
                {
        ...
        public ActionResult Edit(Reserva reserva)
        {
            ...
            using (SqlConnection conexion = cn.ObtenerConexion())
            {
                string query = @"UPDATE Reservas SET IdUsuario=@IdUsuario, IdAreaComun=@IdAreaComun,
